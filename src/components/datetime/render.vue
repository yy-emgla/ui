...
import Manager from './../picker/chain';
import value2name from './../picker/value2name';
...
export default {
  ...
  methods: {
    ...
    dayRander() {
      const vm = this.scroller[2].options.vue;
      vm && (vm.$emit('dayrender'));
    },
    render(data, value) {
      ...
      for (let i = 0; i < data.length; i += 1) {
        ...
        if (ui.scroller[i]) {
          const vm = ui.scroller[i].options.vue;
          vm && (vm.$emit('setdata', data[i]));
          ui.scroller[i].options.data = data[i];
        } else {
          ui.scroller[i] = new Scroller(ui.getId(i), {
            data: data[i],
            defaultValue: value[i] || data[i][0].value || data[i][0],
