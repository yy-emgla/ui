...
import scrollerEl from './scrollel';
...
const Animate = require('./../picker/animate')
const { getElement, getComputedStyle, easeOutCubic, easeInOutCubic } = require('./../picker/util')
...
const Scroller = function (container, options) {
  ...
  const TEMPLATE = `
    <div class="scroller-component" id="_${self.options.index}" data-role="component">
      ...
      <div class="scrollel"  data-role="scrollEl"></div>
    </div>
    `

  self.__container = getElement(container)

  let tempContainer = self.div = document.createElement('div')
  ...
  let El = component.querySelector('[data-role=scrollEl]')
  var indicator = component.querySelector('[data-role=indicator]')

  const scrollElVm = VUE.extend(self.options.scrollerEl)


  this.options.vue = new scrollElVm({propsData: {
      dataArray: self.options.data,
      temClass: self.options.itemClass,
      dayrender: () => {
        self.renderUpdatFunc() // update
      }
    }});
  this.options.vue.$on('setdata', (val) => {
    this.options.vue.array = val;
  });
  this.options.vue.$on('dayrender', (val) => {
    self.renderUpdatFunc() // update
  });
  self.__container.appendChild(component)
  this.options.vue.$mount(El)
  ...
  this.renderUpdatFunc = () => {
    self.__component = document.querySelector(`#_${self.options.index}`)
    self.__content = self.__component.querySelector('[data-role=content]')
    const indicator = self.__component.querySelector('[data-role=indicator]')
    self.__itemHeight = parseFloat(getComputedStyle(indicator, 'height'), 10)

    self.__callback = options.callback || function (top) {
      const distance = -top
      self.__content.style.webkitTransform = 'translate3d(0, ' + distance + 'px, 0)'
      self.__content.style.transform = 'translate3d(0, ' + distance + 'px, 0)'
    }

    var rect = self.__component.getBoundingClientRect()

    self.__clientTop = (rect.top + self.__component.clientTop) || 0
    self.__setDimensions(self.__component.clientHeight, self.__content.offsetHeight)

    if (self.__component.clientHeight === 0) {
      self.__setDimensions(parseFloat(getComputedStyle(self.__component, 'height'), 10), 204)
    }
  }
}
